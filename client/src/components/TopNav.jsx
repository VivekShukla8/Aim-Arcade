        ...
        <div className="flex items-center gap-2">
          ...
          {open && me && (
            <div className="absolute right-0 mt-2 w-48 rounded-xl bg-black/95 backdrop-blur-xl border border-zinc-800 text-sm overflow-hidden shadow-2xl shadow-black/50 animate-in fade-in slide-in-from-top-2 duration-200">
              ...
              <Link to="/profile" className="flex items-center gap-2 px-4 py-2.5 hover:bg-zinc-900/70 transition-colors text-zinc-300">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {mobile && (
        <div className="md:hidden border-t border-zinc-800 bg-black/95 backdrop-blur-xl shadow-2xl">
          <div className="max-w-6xl mx-auto px-4 py-3 flex flex-col gap-1 text-sm">
            {me && (
              <div className="px-3 py-3 rounded-lg bg-zinc-900/50 border border-zinc-800 flex items-center gap-3 mb-1">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center text-xs font-bold">
                  {(me.name?.[0] || me.email?.[0] || 'U').toUpperCase()}
                </div>
                <div className="min-w-0">
                  <div className="text-white text-xs font-medium truncate">{me.name || 'Unnamed User'}</div>
                  <div className="text-zinc-400 text-[11px] truncate">{me.email}</div>
                </div>
              </div>
            )}
            ...
            {me && (
              <>
                <Link onClick={()=>setMobile(false)} to="/profile" className="flex items-center gap-2 px-3 py-3 hover:bg-zinc-900/50 rounded-lg transition-colors text-zinc-300">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A9 9 0 1118.88 6.196M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                  Profile
